pro plot_contour_irs, noiselevel=noiselevel, no_plot=no_plot,indir=indir,plotdir=plotdir,objname=objname,spire=spire,pacs=pacs,verbose=verbose,brightness=brightness,fx=fx,plot_irs2=plot_irs2, $
                      spitzerirs=spitzerirs, max_irs_pixel=max_irs_pixel, irs_mod=irs_mod
...
if keyword_set(brightness) then begin
    ...
    if irs_mod eq 'LH' then beam_irs = 4.5^2
    if irs_mod eq 'SH' then beam_irs = 2.2^2
endif
...
if keyword_set(spitzerirs) then begin
    ; Spitzer-IRS
    readcol, indir+objname+'_irs_pixel1_lines.txt', format='A,D,D,D,D, D,D,D,D,D, D,D, D,A,A,D', name, lab_wl, wl, sig_wl, str, sig_str, fwhm, sig_fwhm, base_str, noise, snr, ra, dec, pixel, blend, validity,/silent,skipline=1
    ; ra_cen_pacs = ra
    ; dec_cen_pacs = dec
    data_irs = replicate({line:strarr(n_elements(name)), ra: dblarr(n_elements(ra)), dec: dblarr(n_elements(dec)), lab_wl:dblarr(n_elements(lab_wl)), wl:dblarr(n_elements(wl)), flux: dblarr(n_elements(str)), flux_sig: dblarr(n_elements(sig_str)),$
        fwhm: dblarr(n_elements(fwhm)), snr: dblarr(n_elements(snr)), base_str: dblarr(n_elements(base_str)), validity: dblarr(n_elements(validity))}, max_irs_pixel)

    for pix = 0, max_irs_pixel-1 do begin
        readcol, indir+objname+'_irs_pixel'+strtrim(string(pix+1),1)+'_lines.txt', format='A,D,D,D,D, D,D,D,D,D, D,D, D,A,A,D', name, lab_wl, wl, sig_wl, str, sig_str, fwhm, sig_fwhm, base_str, noise, snr, ra, dec, pixel, blend, validity,/silent,skipline=1
        if pix eq 0 then begin
          ra_cen_irs = ra
          dec_cen_irs = dec
        endif
    
    data_irs[pix].line = name
    data_irs[pix].lab_wl = lab_wl
    data_irs[pix].wl = wl
    data_irs[pix].flux = str / beam_irs
    data_irs[pix].flux_sig = sig_str / beam_irs
    data_irs[pix].fwhm = fwhm
    data_irs[pix].ra = ra
    data_irs[pix].dec = dec
    data_irs[pix].snr = snr
    data_irs[pix].validity = validity
    for i = 0, n_elements(base_str)-1 do if base_str[i] lt 0 then base_str[i] = 0
    data_irs[pix].base_str = base_str*fwhm / beam_irs ;*2.354    ; Use a top-hat function to calculate the baseline strength under the line
    endfor
...
if not keyword_set(no_plot) then begin
    ...
    for i = 0, n_elements(line_name)-1 do begin
        ...
        if (data_slw[pix].flux[data_ind] lt 0) or (data_slw[pix].snr[data_ind] lt noiselevel) or (data_ind eq -1) or (data_slw[pix].validity[data_ind] eq 0) then begin
        ...
        if max(flux) ne min(flux[where(flux ne 0)]) then begin
                ...
                level = level[where(level ge noiselevel*noise[0])]
                if noiselevel*noise ge max(flux) then stop
            endelse
    ...
    for i = 0, n_elements(line_name)-1 do begin
        ...
        ; exclude absorption lines
        ; set every absorption line to zero
        if (data_ssw[pix].flux[data_ind] lt 0) or (data_ssw[pix].snr[data_ind] lt noiselevel) or (data_ind eq -1) or (data_ssw[pix].validity[data_ind] eq 0) then begin
    ...
    for i = 0, n_elements(line_name)-1 do begin
        ...
        ; exclude absorption lines
        ; set every absorption line to zero
                if (data_pacs[pix].flux[data_ind] lt 0) or (data_pacs[pix].snr[data_ind] lt noiselevel) or (data_ind eq -1) or (data_pacs[pix].validity[data_ind] eq 0) then begin
    ...
    if keyword_set(spitzerirs) then begin
      ...
      line_name =  ['H2S2', 'NeII12', 'NeIII15', 'H2S1', 'FeII18',$
                    'SIII18','FeIII23', 'SI25',$
                    'FeII24', 'FeII26', 'H2S0',$
                    'SIII33', 'SiII34', 'FeII35', 'NeIII36']

      if keyword_set(verbose) then print, 'contour plots for Spitzer-IRS'
      for i = 0, n_elements(line_name)-1 do begin
        wl = []
        flux = []
        flux_sig = []
        ...
        dec = []
        dec_tot = []
        for pix = 0, n_elements(data_irs[*].ra[0])-1 do begin
          ; read in the central position
          if pix eq 0 then begin
            ra_cen = ra_cen_irs[where(data_irs[pix].line eq line_name[i])]
            dec_cen = dec_cen_irs[where(data_irs[pix].line eq line_name[i])]
          endif
          data_ind = where(data_irs[pix].line eq line_name[i])
          ra_tot = [ra_tot, data_irs[pix].ra[data_ind]]
          dec_tot = [dec_tot, data_irs[pix].dec[data_ind]]
          base_str = [base_str, data_irs[pix].base_str[data_ind]]
          
          ; exclude absorption lines
          ; set every absorption line to zero
          ; if (data_irs[pix].flux[data_ind] lt 0) or (data_irs[pix].snr[data_ind] lt noiselevel) or (data_ind eq -1) or (data_irs[pix].validity[data_ind] eq 0) then begin
          if (data_irs[pix].flux[data_ind] lt 0) or (data_irs[pix].snr[data_ind] lt noiselevel) or (data_ind eq -1) then begin
            wl = [wl, data_irs[pix].wl[data_ind]]
            flux = [flux, 0]
            flux_sig = [flux_sig, 0]
            snr = [snr, data_irs[pix].snr[data_ind]]
            ra = [ra, data_irs[pix].ra[data_ind]]
            dec = [dec, data_irs[pix].dec[data_ind]]
          endif else begin
            wl = [wl, data_irs[pix].wl[data_ind]]
            flux = [flux, data_irs[pix].flux[data_ind]]
            flux_sig = [flux_sig, data_irs[pix].flux_sig[data_ind]]
            snr = [snr, data_irs[pix].snr[data_ind]]
            ra = [ra, data_irs[pix].ra[data_ind]]
            dec = [dec, data_irs[pix].dec[data_ind]]
          endelse
        ...
        if (n_elements(flux[where(flux ne 0)]) ge 1) and ((where(flux ne 0))[0] ne -1) then begin
          ...
          plotposition = [0.15,0.15,0.95,0.9]
          ra_range = max(ra_tot_smooth[*,0])-min(ra_tot_smooth[*,0])
          dec_range = max(dec_tot_smooth[0,*])-min(dec_tot_smooth[0,*])
          position = [0.5*(ra_range-max(ra_tot_smooth[*,0]))/ra_range+plotposition[0], 0.5*(dec_range+min(dec_tot_smooth[0,*]))/dec_range+plotposition[1],$
            plotposition[2]-0.5*(min(ra_tot_smooth[*,0])+ra_range)/ra_range, plotposition[3]-0.5*(dec_range-max(dec_tot_smooth[0,*]))/dec_range]
         ;  position = plotposition

          loadct, 13, /silent
          cgplot, ra_tot, dec_tot, psym=1,xrange=[max(ra_tot_smooth[*,0]), min(ra_tot_smooth[*,0])],$
                  yrange=[min(dec_tot_smooth[0,*]), max(dec_tot_smooth[0,*])], position=plotposition,/nodata,color=255,xtitle='!nRA offset (arcsec)',ytitle='!nDec offset (arcsec)',aspect=1.0
          colorFile = '~/programs/misc/fsc_brewer.tbl';Filepath(SUBDIRECTORY=['resource','colors'], 'fsc_brewer.tbl')
          cgloadct, 10, /reverse, file=colorfile, /silent
          cgimage, base_str_smooth/max(base_str_smooth)*255,/overplot,$
            oposition=oposition,/keep_aspect_ratio,alphafgposition=position,$;, xrange=[30,-30], yrange=[-30,30]
